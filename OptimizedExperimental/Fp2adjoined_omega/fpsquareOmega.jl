...
function Fp2(p::T) where T <: Integer
    ...
    mod(p, 3) != 2 && error("p must be 2 (mod 3)")
...
function *(x::Fp2Elem{T}, y::Fp2Elem{T}) where T <: Integer
    ...
    b_new = mod((x.a*y.b + x.b*(y.a - y.b)), x.field.p)
    return x.field(a_new, b_new)
end
...
function inv(x::Fp2Elem{T}) where T <: Integer
    ...
    if x.a == 0
        a_new = 1*invmod(-x.b,x.field.p)
        return x.field(a_new, a_new)
    end
    b2 = x.b*x.b
    abx = x.a*x.b
    c = x.a*x.a + b2 - abx
